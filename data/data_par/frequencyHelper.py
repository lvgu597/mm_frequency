...
if __name__ == '__main__':
    file_json = '../../../tianchidata_coco_base/annotations/instances_train0331.json'
    ...
    for times in range(0, len(file_names_all), 10):
        ...
        train_images = np.zeros((len(file_names), 1920, 2560, 3))
        file_root = '../../../tianchidata_coco_base/defect_images/'
        for i in range(len(file_names)):
            train_images[i, :] = cv.imread(f'{file_root}{file_names[i]}')
        for r in [120,480]: #240, 480, 720, 960, 1200, 1440, 1680, 1920
            save_dir = '../../../tianchidata_coco_base/'
            # if not os.path.exists(f'{save_dir}train_image_low_{r}'):
            #     os.mkdir(f'{save_dir}train_image_low_{r}')
            if not os.path.exists(f'{save_dir}train_image_high_{r}'):
                os.mkdir(f'{save_dir}train_image_high_{r}')
            train_image_low, train_image_high = generateDataWithDifferentFrequencies_3Channel(train_images, r)
            for i in range(len(file_names)):
                # cv.imwrite(f'{save_dir}train_image_low_{r}/{file_names[i]}',train_image_low[i,:])
                cv.imwrite(f'{save_dir}train_image_high_{r}/{file_names[i]}',train_image_high[i,:])
